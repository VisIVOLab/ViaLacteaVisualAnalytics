...
*   Summary of routines
*   -------------------
*   wcsfwd() and libwcsrev() are high level driver routines for the WCS linear
...
*   unknown elements.
*
*   An initialization routine, libwcsset(), computes indices from the ctype
*   array but need not be called explicitly - see the explanation of
*   wcs.flag below.
*
*
*   Initialization routine; libwcsset()
*   --------------------------------
...
*   it will be invoked by wcsfwd() and libwcsrev() if the "flag" structure member
...
*
*
*   Reverse transformation; libwcsrev()
*   --------------------------------
...
*             0 to 5 as above.
*
*       These routines support both methods; libwcsset() determines which is
*       being used by the presence or absence of a CUBEFACE axis in ctype[].
*       wcsfwd() and libwcsrev() translate the CUBEFACE axis representation to
...
*         libwcsset() to redetermine the indices.  The flag is set to 999 if
...

{
   ...
   if (wcs->flag != WCSSET) {
      if (libwcsset(lin->naxis, ctype, wcs)) return 1;
   }
...
int
libwcsrev(ctype, wcs, pixcrd, lin, imgcrd, prj, phi, theta, crval, cel, world)
