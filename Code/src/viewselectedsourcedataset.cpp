...
ViewSelectedSourceDataset::ViewSelectedSourceDataset(vtkwindow_new *v,
                                                     QHash<int, QString> selFields,
                                                     QList<QListWidgetItem *> selSources,
                                                     QWidget *parent)
    : QWidget(parent), ui(new Ui::ViewSelectedSourceDataset)
{
    ...
    for (int j = 0; j < selFields.size(); j++) {
        ui->datasetTableWidget->setHorizontalHeaderItem(
                j, new QTableWidgetItem(selFields.values().at(j)));
    }
    ...
    for (int i = 0; i < selSources.size(); i++) {
        ...
        for (int j = 0; j < selFields.size(); j++) {
            QTableWidgetItem *item = new QTableWidgetItem(
                    QString::fromStdString(table->getTableData()[selFields.keys()[j]][row]));
