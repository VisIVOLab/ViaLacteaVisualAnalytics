...
void vtkInteractorStyleImageCustom::OnMouseMove()
{
    ...
    float *pixel =
            this->reader->GetPixelValue(world_coord[0], world_coord[1], this->reader->GetSlice());
    oss << "<value> ";
    if (pixel != NULL)
        oss << pixel[0];
    else
    ...
    std::string filepath(this->reader->GetFileName());
    // Galactic coords
...
//----------------------------------------------------------------------------
void vtkInteractorStyleImageCustom::SetReader(vtkSmartPointer<vtkFitsReader2> reader)
{
    this->reader = reader;
}
