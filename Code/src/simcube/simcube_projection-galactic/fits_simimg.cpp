...
fits_simimg::fits_simimg(string fits_filename):fptr{nullptr}
{
	...
	{
		throw std::invalid_argument("Error: reading CDELT 1 2 : " + get_errstatus(status));
	}
}
...
void fits_simimg::update_card(string keyname, string keyvalue)
{
	...
	{
		throw invalid_argument("Keys to update must exist but read card " + string(card) + " yields error : " + get_errstatus(status));
	}
	...
	{
		throw invalid_argument("Protected keyword cannot be modified: " + string(card));
	}
	else
	{
		...
		{
			throw invalid_argument("Card has already the expected value: " + string(card));
		}
	...
	if(status) throw runtime_error("update_card failed: " + get_errstatus(status));
}
...
void fits_simimg::update_header(const double cdelt_deg[2], const double crval_deg[2])
{
   map<string, string> keys_const_value
        ...
        for(map<string, string>::iterator it = keys_const_value.begin(); it!= keys_const_value.end(); ++it)
	...
	for(map<string, string>::iterator it = keys_calc_value.begin(); it!= keys_calc_value.end(); ++it)
                update_card(it->first, it->second);
