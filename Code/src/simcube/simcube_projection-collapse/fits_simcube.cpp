...
void assert_cunit3_is_homogeneous(fitsfile *fptr)
{
    ...
    for (ii = 0; ii < 3; ii++)
        cunit[ii] = (char *)malloc(FLEN_VALUE);
...
void read_cdelt3(fitsfile *fptr, double cdelt[3])
{
    ...
    if (nfound != 3)
        throw invalid_argument("CDELT key(s) missing");
}

// class

fits_simcube::fits_simcube(std::string fits_filename) : pix_row { nullptr }, fptr { nullptr }
{
    int status = 0;
