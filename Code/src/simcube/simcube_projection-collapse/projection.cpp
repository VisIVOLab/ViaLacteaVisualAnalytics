...
void projection::rotation_matrix_with_scaling(double srotm[2][3], const double rotation_angle_deg[3], const double cdelt[3])
{
        ...
        {
                {cdelt[0] * (cA*cB),    cdelt[1] * (cA*sB*sG - sA*cG),  cdelt[2] * (cA*sB*cG + sA*sG)},
                {cdelt[0] * (sA*cB),    cdelt[1] * (sA*sB*sG + cA*cG),  cdelt[2] * (sA*sB*cG - cA*sG)}
...
//====================================================================================================
struct projection::point2d projection::rotate(const double rotm[2][3], const double pixel[3], const double center[3])
{
...
//====================================================================================================
struct projection::xy_bounds projection::bounds(const double rotmx[2][3],
		const long naxis[3],
...
*/
		...
		struct projection::point2d xy_parsec{projection::rotate(rotmx, vvv, rot_center_pixels)};

		if(xy_parsec.x < bnds.x_min) bnds.x_min = xy_parsec.x;//xx;
		if(xy_parsec.x > bnds.x_max) bnds.x_max = xy_parsec.x;//xx;
		if(xy_parsec.y < bnds.y_min) bnds.y_min = xy_parsec.y;//yy;
		if(xy_parsec.y > bnds.y_max) bnds.y_max = xy_parsec.y;//yy;
	}
	return bnds;
}
