...
double max_abs_cdelt(const double cdelt[3], double scale = 1.0)
{
    ...
    for (int i = 0; i < 3; i++)
        if (abs(cdelt[i]) > max_cdelt)
            max_cdelt = abs(cdelt[i]);
    return max_cdelt * scale;
}

void simcube::rotate_and_collapse(std::string fitsname_image_out, std::string fitsname_cube_in,
                                  const double rotation_angles_deg[3], double scale)
{
    ...
    double rot_center[3] { double(f.naxes[0] + 1) / 2.0, double(f.naxes[1] + 1) / 2.0,
                           double(f.naxes[2] + 1) / 2.0 };
    ...
    for (fpixel[2] = 1; fpixel[2] <= f.naxes[2]; fpixel[2]++)
        for (fpixel[1] = 1; fpixel[1] <= f.naxes[1]; fpixel[1]++) {
            ...
            for (int ii = 1; ii <= f.naxes[0]; ii++) {
                ...
                {
                    projection::rotate(srotm, vvv, rot_center)
                };
