...
//----------------------------------------------------------------------------
int ExtendedGlyph3D::RequestData(vtkInformation *vtkNotUsed(request),
...
//----------------------------------------------------------------------------
bool ExtendedGlyph3D::Execute(vtkDataSet *input, vtkInformationVector *sourceVector,
                              vtkPolyData *output, int requestedGhostLevel)
{
    ...
    for (inPtId = 0; inPtId < numPts; inPtId++) {
        ...
        if (pd) {
            ...
            outputPD->CopyData(pd, srcPointIdList.GetPointer(), dstPointIdList.GetPointer());
            if (this->FillCellData) {
                for (i = 0; i < numSourceCells; ++i) {
                    srcCellIdList->SetId(i, inPtId);
                    dstCellIdList->SetId(i, cellIncr + i);
                }
                outputCD->CopyData(pd, srcCellIdList.GetPointer(), dstCellIdList.GetPointer());
            }
        }
...
void ExtendedGlyph3D::PrintSelf(ostream& os, vtkIndent indent)
{
    ...
    {
        os << indent << "A table of " << this->GetNumberOfInputConnections(1) << " glyphs has been
defined\n";
    }
...
*/
int ExtendedGlyph3D::RequestUpdateExtent(vtkInformation *vtkNotUsed(request),
                                         vtkInformationVector **inputVector,
                                         vtkInformationVector *outputVector)
{
    // get the info objects
