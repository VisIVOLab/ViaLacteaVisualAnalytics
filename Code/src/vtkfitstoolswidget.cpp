...
vtkfitstoolswidget::vtkfitstoolswidget(vtkwindow_new *v, QWidget *parent)
    : QWidget(parent), ui(new Ui::vtkfitstoolswidget)
{
    ui->setupUi(this);
    ...
    QObject::connect(shortcut_local, SIGNAL(activated()), this,
                     SLOT(on_addLocalSourcesPushButton_clicked()));

    QShortcut *shortcut_remote = new QShortcut(QKeySequence(Qt::CTRL + Qt::Key_R), this);
    QObject::connect(shortcut_remote, SIGNAL(activated()), this,
                     SLOT(on_addRemoteSourcesPushButton_clicked()));

    /*
    QShortcut *shortcut_dataCube_remote = new QShortcut(QKeySequence(Qt::CTRL + Qt::Key_D) , this);
    QObject::connect(shortcut_dataCube_remote, SIGNAL(activated()), this,
    SLOT(on_datacubeButton_clicked()));
*/

    ui->addedSourcesListWidget->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(ui->addedSourcesListWidget, SIGNAL(customContextMenuRequested(QPoint)),
...
void vtkfitstoolswidget::deleteSelected()
{
    ...
    if (!select->selectedRows().isEmpty()) {
        ...
        vtkwin->removeSingleEllipse(vtkwin->getEllipseActorList().value(
                ui->addedSourcesListWidget->item(row, 1)->text()));
...
void vtkfitstoolswidget::opacityValueChanged(int o)
{
    ...
    vtkwin->getEllipseActorList()
            .value(ui->addedSourcesListWidget->item(o, 1)->text())
            ->GetProperty()
            ->SetOpacity(opacity);
    vtkwin->update();
}
...
void vtkfitstoolswidget::checkboxClicked(int cb)
{
    ...
    if (vtkwin->getEllipseActorList()
                .value(ui->addedSourcesListWidget->item(cb, 1)->text())
                ->GetVisibility())
        vtkwin->getEllipseActorList()
                .value(ui->addedSourcesListWidget->item(cb, 1)->text())
                ->VisibilityOff();
                                      ...
                                      void *vtkNotUsed(clientData), void *callData)
{
    ...
    std::cout << "Start x: " << pos1X << " Start y: " << pos1Y << " End x: " << pos2X
...
void vtkfitstoolswidget::on_lutComboBox_currentIndexChanged(int index)
{
    ...
    changeLutScale(ui->lutComboBox->currentText().toStdString().c_str(),
                   ui->scaleComboBox->currentText().toStdString().c_str());
}
...
void vtkfitstoolswidget::on_addedSourcesListWidget_doubleClicked(const QModelIndex &index)
{
    ...
    if (index.column() == 0) {
        ...
        double r = vtkwin->getEllipseActorList()
                           .value(ui->addedSourcesListWidget->item(index.row(), 1)->text())
                           ->GetProperty()
                           ->GetColor()[0]
                * 255;
        ...
        ui->addedSourcesListWidget->cellWidget(index.row(), 0)
