...
void HigalSelectedSources::itemPressed(QListWidgetItem *cur)
{
    ...
    if (itemChanged) {
        ...
        // ::atof(table->getTableData()[table->getColId("fwhma")][row].c_str());
...
void HigalSelectedSources::sourceChangedEvent(QListWidgetItem *cur, QListWidgetItem *pre)
{
    ...
    if(itemSelected)
    {
        ...
        int row = vtkwin->getEllipseList().value(cur->text())->getRowInTable();
        double semiMajorAxisLength=
    ::atof(table->getTableData()[table->getColId("fwhma")][row].c_str()); double
    semiMinorAxisLength= ::atof( table->getTableData()[table->getColId("fwhmb")][row].c_str());
        double angle= ::atof(table->getTableData()[table->getColId("pa")][row].c_str());
        double ra= ::atof(table->getTableData()[table->getColId("glon")][row].c_str());
        double dec= ::atof(table->getTableData()[table->getColId("glat")][row].c_str());
        ...
        vtkEllipse *el= new vtkEllipse(semiMajorAxisLength/2,semiMinorAxisLength/2,angle, coord[0],
    coord[1], coord[2], 0,0, cur->text(), NULL); drawSingleEllipse(el);
        */
}
...
void HigalSelectedSources::on_sedButton_clicked()
{
    ...
    qDebug() << "SELECTED: " << ui->tabWidget->tabText(ui->tabWidget->currentIndex()) << " -> "
             << wave;
    ...
    for (int i = 0; i < selectedItems.size(); i++) {
        ...
        // designation"+wave;
        //    QString query="SELECT * FROM vlkb_compactsources.sed_view_final where
        //    designation"+wave;
        ...
        // QString query="SELECT 1 as flux22, 0.2 as err_flux22, * FROM
        // vlkb_compactsources.sed_view_final where designationft=";
